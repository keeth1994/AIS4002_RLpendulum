...
class RotaryPendulumParams:
    ...
    pendulum_mass: float = 0.053
    pendulum_length: float = 0.128
    arm_damping: float = 0.0004
    pendulum_damping: float = 0.000004
    motor_resistance: float = 8.94
    motor_torque_constant: float = 0.042
    motor_back_emf_constant: float = 0.0431
...
class RotaryPendulumEnv(gym.Env):
    ...
    def step(self, action: np.ndarray | list[float] | float) -> tuple[np.ndarray, float, bool, bool, dict]:
        ...
        reward = (
            4.0 * np.cos(alpha_error) + 4.0 *float(abs(alpha_error)< np.deg2rad(15))
            - 0.8 * theta**2
            - 0.002 * theta_dot**2
            - 0.001 * alpha_dot**2
            - 0.0002 * voltage**2
